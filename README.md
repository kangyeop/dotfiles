...
`zsh/zshrc`는 심링크라서 이 기기에서만 필요한 설정을 직접 적어 넣으면 다른 기기까지 따라간다. 그래서 `.zshrc` 맨 끝에서 `~/.zshrc.local`을 있으면 source 하도록 해뒀다. 이 파일은 레포가 추적하지 않으므로 회사 전용 PATH, 비공개 환경변수, 그 기기에서만 쓰는 alias 같은 것을 여기에 둔다. 가장 마지막에 로드되므로 `.zshrc`의 값을 덮어쓸 수도 있다.
...
Claude/Codex 설정(`claude/settings.json`, `codex/config.toml`)은 심링크가 아니라 최초 1회 복사되는 템플릿이라 이미 기기별로 따로 놀 수 있다. 별도의 `.local` 파일이 필요 없다.
...
| (없음) | `~/.zshrc.local` — 존재하면 `.zshrc` 끝에서 source하는 기기별 로컬 설정 |
